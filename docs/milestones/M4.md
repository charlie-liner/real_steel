...
Every frame produces one row in a Parquet file. The schema captures raw MediaPipe output, computed angles, robot commands, robot actual state, and gap metrics. Nothing more.
...
### Frame Record (one row per frame in Parquet)
...
columns:
  ...
  - lm_{0..32}_x: float32          # 33 landmark x coordinates (world)
  - lm_{0..32}_y: float32          # 33 landmark y coordinates (world)
  - lm_{0..32}_z: float32          # 33 landmark z coordinates (world)
  - lm_{0..32}_vis: float32        # 33 landmark visibility scores
  - pose_confidence: float32       # average visibility of 6 upper-body keypoints

  # Stage 2: Computed human angles
  - human_angle_{0..7}: float32    # 8 joint angles (radians)

  # Stage 3: Robot command and actual state
  - robot_cmd_{0..7}: float32      # commanded angles (radians)
  - robot_actual_{0..7}: float32   # actual angles from simulator (radians)

  # Gap B (computed per frame)
  - gap_b_{0..7}: float32          # |robot_cmd - robot_actual| per joint (radians)
  - gap_b_rmse: float32            # RMSE across all joints (radians)

  # Performance
  - latency_ms: float32            # total pipeline latency

  # Motion context (optional, set during guided test sequences)
  - motion_label: string            # e.g. "jab", "cross", null for free motion
  - motion_phase: string            # e.g. "guard", "extend", "retract", null
```
...
data/
├── sessions/
│   ├── 2024-01-15_14-30-00.parquet     # one file per session
│   ├── 2024-01-15_15-00-00.parquet
│   └── ...
└── benchmarks/
...
### Session Metadata (embedded in Parquet file metadata)
...
|---|----------|------|-------------|
| D1 | Recorder | `src/evaluation/recorder.py` | Collects frame data and writes to Parquet |
...
Records pipeline data to Parquet files.
...
2. Initialize an empty list for frame records
3. Store config snapshot for embedding in Parquet metadata
4. Return session ID

**`record_frame(...)`:**
1. Compute `gap_b = np.abs(robot_cmd - robot_actual)` per joint
2. Compute `gap_b_rmse = np.sqrt(np.mean(gap_b ** 2))`
3. Flatten landmarks to individual columns (if not None, else fill with NaN)
4. Append dict to frame buffer
5. If buffer reaches 1000 rows, flush to intermediate buffer (memory management)

**`end_session()`:**
1. Convert all buffered records to pandas DataFrame
2. Compute session summary stats (avg gap, avg latency, etc.)
3. Write to `{output_dir}/{session_id}.parquet` with metadata embedded
4. Return file path

**Buffer strategy:**
- Accumulate rows in a list of dicts (fast append)
- Flush to DataFrame only on `end_session()`
- For sessions > 10 minutes (~30,000 frames), this uses ~100MB RAM — acceptable
...
class Replay:
    ...
    def load_session(self, parquet_path: str) -> pd.DataFrame
...
**`load_session(parquet_path)`:**
1. `pd.read_parquet(parquet_path)`
2. Return DataFrame

**`replay_with_config(session_df, new_config)`:**
1. Extract stored landmarks from each row
...
5. Return new DataFrame with recalculated values

**`compare(original_df, replayed_df)`:**
...
python -m src.evaluation.replay \
    --session data/sessions/2024-01-15_14-30-00.parquet \
...
**Done when:** Unit test records 100 frames and writes a valid Parquet file
...
- `test_recorder_creates_parquet`: Record N frames, verify Parquet file exists and has correct columns
- `test_recorder_metadata`: Verify session metadata is embedded in Parquet file
...
- Session data saved to Parquet after completion
...
3. AGENT:
   a. Loads parquet file, reads summary stats
...
- [ ] `python src/main.py --sim --record` saves session data to `data/sessions/*.parquet`
- [ ] Parquet file contains all schema columns with correct dtypes
- [ ] Session metadata embedded in Parquet file
...
- [ ] Recording a 2-minute session produces a Parquet file < 50MB
