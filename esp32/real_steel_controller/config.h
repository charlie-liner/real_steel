...
const uint8_t SERVO_CHANNEL[NUM_SERVOS] = {0, 1, 2, 3, 4, 5, 6};

// Servo pulse width limits (microseconds).
// CALIBRATE THESE per servo — defaults are conservative.
const int SERVO_MIN_US[NUM_SERVOS] = {500, 500, 500, 500, 500, 500, 500};
const int SERVO_MAX_US[NUM_SERVOS] = {2500, 2500, 2500, 2500, 2500, 2500, 2500};

// Angle limits (degrees) — must match motion_mapper joint limits.
//                              tilt   pan  elbow  tilt   pan  elbow  torso
const float ANGLE_MIN[NUM_SERVOS] = {-90,  -90,    0,  -90,  -90,    0,  -90};
const float ANGLE_MAX[NUM_SERVOS] = { 90,   90,  135,   90,   90,  135,   90};

// Home positions (degrees)
const float HOME_ANGLES[NUM_SERVOS] = {0, 0, 0, 0, 0, 0, 0};
